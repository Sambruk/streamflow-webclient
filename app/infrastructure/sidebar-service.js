...
angular.module('sf').factory('sidebarService', function ($routeParams, $route, caseService, $q, $rootScope, $location, navigationService, tokenService, checkPermissionService) {
    ...
    var _openForm = function (scope, formId, isNewWindow, isEmpty) {
        if (isNewWindow) {
            ...
            var url = '#/cases/' + scope.caze[0].id + (isEmpty?'/formdrafts/':'/formhistory/') + formId+'?isFormWindow=true';
            var popupWindow = window.open(url, 'FormWindow_' + Math.random(), 'height=' + height * 0.7 + ', width=' + width / 2.17 + ', left=' + width * 0.33 + ', scrollbars=yes');
            if (window.focus) {
                popupWindow.focus();
            }
            _updateToolbar(scope);
